...
public static partial class RewrittenLiquidRenderer
{
    ...
    private static Rectangle _drawArea = new(0, 0, 1, 1);
    private static readonly UnifiedRandom _random = new();
    ...
    */
    ...
    private static unsafe void InternalPrepareDraw(LiquidRenderer unusedArg0Filler, Rectangle drawArea)
    {
        ...
        */
        ...
        fixed (LiquidCache* ptr = &_cache[1])
        {
            ...
            var num2 = 0f;
            ptr2 += num;
            for (var k = 2; k < rectangle.Width - 2; k++)
            {
                for (var l = 2; l < rectangle.Height - 2; l++)
                {
                    num2 = 0f;
                    ...
                    else if (!ptr2->HasLiquid && !ptr2->EdgeData.HasValue)
                    {
                        ...
                        var liquidCache4 = ptr2[rectangle.Height];
            ...
            ptr2 = ptr;
            for (var m = 0; m < rectangle.Width; m++)
            {
            ...
            ptr2 = ptr;
            ptr2 += num;
            for (var num6 = 2; num6 < rectangle.Width - 2; num6++)
            {
                for (var num7 = 2; num7 < rectangle.Height - 2; num7++)
                {
            ...
            ptr2 = ptr;
            ptr2 += num;
            for (var num12 = 2; num12 < rectangle.Width - 2; num12++)
            {
                for (var num13 = 2; num13 < rectangle.Height - 2; num13++)
                {
                    if (ptr2->HasVisibleLiquid)
                    {
                        var liquidCache = ptr2[-1];
            ...
            ptr2 = ptr;
            ptr2 += num;
            for (var num14 = 2; num14 < rectangle.Width - 2; num14++)
            {
                for (var num15 = 2; num15 < rectangle.Height - 2; num15++)
                {
                    if (ptr2->HasLiquid)
                    {
                        var liquidCache = ptr2[-1];
            ...
            ptr2 = ptr;
            ptr2 += num;
            for (var num16 = 2; num16 < rectangle.Width - 2; num16++)
            {
                for (var num17 = 2; num17 < rectangle.Height - 2; num17++)
                {
                    if (ptr2->HasLiquid)
                    {
                        var liquidCache = ptr2[-1];
            ...
            fixed (LiquidDrawCache* ptr3 = &_drawCache[0])
            {
                fixed (Color* ptr5 = &_waveMask[0])
                {
                    var ptr4 = ptr3;
                    var ptr6 = ptr5;
                    for (var num18 = 2; num18 < rectangle.Width - 2; num18++)
                    {
                        for (var num19 = 2; num19 < rectangle.Height - 2; num19++)
                        {
                            if (ptr2->HasVisibleLiquid)
                            {
                                var num20 = Math.Min(0.75f, ptr2->VisibleLeftWall);
                                ...
                                var b = LiquidRenderer.WAVE_MASK_STRENGTH[ptr2->VisibleType];
                                var g = ptr6->R = (byte)(b >> 1);
                                ptr6->G = g;
                            ...
                            else
                            {
                                ...
                                var num24 = !ptr2->IsSolid && !ptr2->IsHalfBrick ? 4 : 3;
                                var b3 = LiquidRenderer.WAVE_MASK_STRENGTH[num24];
                                var g2 = ptr6->R = (byte)(b3 >> 1);
                                ptr6->G = g2;
            ...
            ptr2 = ptr;
            for (var num25 = rectangle.X; num25 < rectangle.X + rectangle.Width; num25++)
            {
                for (var num26 = rectangle.Y; num26 < rectangle.Y + rectangle.Height; num26++)
                {
                    if (ptr2->VisibleType == 1 && ptr2->HasVisibleLiquid && Dust.lavaBubbles < 200)
                    {
                        ...
                        {
                            var num27 = Dust.NewDust(new Vector2(num25 * 16, num26 * 16), 16, 8, 35, 0f, 0f, 50, Color.White, 1.5f);
                            Main.dust[num27].velocity *= 0.8f;
                            Main.dust[num27].velocity.X *= 2f;
                            Main.dust[num27].velocity.Y -= _random.Next(1, 7) * 0.1f;
    ...
    public static unsafe void DrawNormalLiquids(LiquidRenderer unusedArg0Filler, SpriteBatch spriteBatch, Vector2 drawOffset, int waterStyle, float globalAlpha, bool isBackgroundDraw)
    {
        ...
        fixed (LiquidDrawCache* ptr = &_drawCache[0])
        {
            ...
            */
            {
                var cacheLength = _drawCache.Length;
                for (var k = 0; k < cacheLength; k++)
                {
                    ...
                    if (ptr2->IsVisible && ptr2->Type != LiquidID.Shimmer)
                    {
                        ...
                        var num = ptr2->Opacity * (isBackgroundDraw ? 1f : LiquidRenderer.DEFAULT_OPACITY[ptr2->Type]);
                        int num2 = ptr2->Type;
                        switch (num2)
                        {
                            case 0:
                        ...
                        var i = ptr2->X + drawArea.X - 2;
                        var j = ptr2->Y + drawArea.Y - 2;
                        num = Math.Min(1f, num);
        ...
        fixed (LiquidDrawCache* ptr = &_drawCache[0])
        {
            ...
            for (var i = 0; i < num; i++)
            {
                ...
                */
                ...
                var liquidOffset = ptr2->LiquidOffset;
                var val = ptr2->Opacity * (isBackgroundDraw ? 1f : 0.75f);
                var num2 = 14;
                val = Math.Min(1f, val);
                var num3 = ptr2->X + drawArea.X - 2;
                var num4 = ptr2->Y + drawArea.Y - 2;
                ...
                var flag = sourceRectangle.X != 16 || sourceRectangle.Y % 80 != 48;
    ...
    public static void Update(LiquidRenderer unusedArg0Filler, GameTime gameTime)
    {
        ...
        {
            var num = Main.windSpeedCurrent * 25f;
